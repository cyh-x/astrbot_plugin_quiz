from astrbot.api.event import filter, AstrMessageEvent, MessageEventResult
from astrbot.api.star import Context, Star, register
from astrbot.api import logger
from astrbot.api import AstrBotConfig
import astrbot.api.message_components as Comp
from astrbot.core.utils.session_waiter import (
    session_waiter,
    SessionController,
)


import os
import json
import random
import re
import asyncio
import aiohttp
from bs4 import BeautifulSoup
import urllib.parse
import shutil
from typing import Dict, Tuple
import logging


async def download_and_parse_quiz(
    quiz_type: str, html_file: str = None, json_file: str = None, save_html: bool = True
):
    """
    异步下载指定测验类型的 HTML 页面，提取题目信息，保存为 JSON。

    Args:
        quiz_type (str): 测验类型，例如 'marvel', 'bleach' 等。
        html_file (str, optional): 保存 HTML 的文件名。若不指定，自动生成 f"{quiz_type}_quiz.html"。
        json_file (str, optional): 保存 JSON 的文件名。若不指定，自动生成 f"{quiz_type}_quiz.json"。
        save_html (bool, optional): 是否保存 HTML 文件到磁盘，默认保存。
    """
    if html_file is None:
        html_file = f"{quiz_type}_quiz.html"
    if json_file is None:
        json_file = f"{quiz_type}_quiz.json"

    url = f"https://foxquiz.com/zh/{quiz_type}-trivia-quiz"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
    }

    # 下载 HTML
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
                response.raise_for_status()
                html_content = await response.text()
        if save_html:
            with open(html_file, "w", encoding="utf-8") as f:
                f.write(html_content)
            logger.info(f"HTML 已保存至 {html_file}")
    except Exception as e:
        logger.info(f"下载 HTML 失败: {e}")
        if os.path.isfile(html_file):
            logger.info(f"使用本地文件 {html_file}")
            with open(html_file, "r", encoding="utf-8") as f:
                html_content = f.read()
        else:
            raise

    # 解析 HTML
    soup = BeautifulSoup(html_content, "html.parser")
    cards = soup.select(
        "div.bg-white.rounded-lg.shadow-lg.overflow-hidden.border.border-gray-200"
    )

    quiz_data = []
    for idx, card in enumerate(cards, 1):
        h3 = card.select_one("h3.text-xl.font-semibold")
        if not h3:
            continue
        full_text = h3.get_text(strip=True)
        match = re.match(r"^(\d+)\s*(.*)", full_text)
        if match:
            number = match.group(1)
            question_text = match.group(2).strip()
        else:
            number = str(idx)
            question_text = full_text

        options = []
        correct_letter = None
        option_divs = card.select("div.p-4.rounded-lg.border")
        for opt in option_divs:
            letter_span = opt.select_one("span.leading-none")
            if not letter_span:
                continue
            letter = letter_span.get_text(strip=True)
            text_p = opt.select_one("p.text-lg")
            option_text = text_p.get_text(strip=True) if text_p else ""
            options.append({"letter": letter, "text": option_text})
            classes = opt.get("class", [])
            if "bg-green-50" in classes and opt.select_one("svg.lucide-check"):
                correct_letter = letter

        image_path = f"{idx}.png"  # 图片文件名与 get_img.py 下载的命名一致
        quiz_data.append(
            {
                "id": idx,
                "number": number,
                "question": question_text,
                "options": options,
                "correct_letter": correct_letter,
                "image": image_path,
            }
        )

    with open(json_file, "w", encoding="utf-8") as f:
        json.dump(quiz_data, f, ensure_ascii=False, indent=2)

    logger.info(f"成功提取 {len(quiz_data)} 道题目，保存至 {json_file}")


BASE_URL = "https://foxquiz.com"


async def download_images(quiz_type: str, output_dir: str = None):
    """
    异步下载指定测验类型的所有图片。

    Args:
        quiz_type (str): 测验类型，例如 'marvel', 'bleach' 等。
        output_dir (str, optional): 图片保存目录。若不指定，则自动生成为 f"{quiz_type}_quiz_images"。
    """
    plugin_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.dirname(
        os.path.dirname(os.path.dirname(plugin_dir))
    )  # AstrBot 根目录
    data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")
    html_file = os.path.join(data_root, f"{quiz_type}_quiz.html")
    if output_dir is None:
        output_dir = f"{quiz_type}_quiz_images"
    os.makedirs(output_dir, exist_ok=True)

    url = f"https://foxquiz.com/zh/{quiz_type}-trivia-quiz"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
    }

    # 获取 HTML
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
                response.raise_for_status()
                html_content = await response.text()
    except Exception as e:
        logger.info(f"获取网页失败: {e}")
        # 如果网络请求失败，尝试读取本地备份文件（可选）
        if os.path.isfile(html_file):
            with open(html_file, "r", encoding="utf-8") as f:
                html_content = f.read()
            logger.info(f"使用本地HTML文件: {html_file}")
        else:
            logger.error(f"未找到{quiz_type}的本地html文件")
            raise

    # 解析 HTML
    soup = BeautifulSoup(html_content, "html.parser")
    cards = soup.find_all(
        "div",
        class_="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200",
    )

    # 下载图片
    async with aiohttp.ClientSession() as session:
        for card in cards:
            # 提取序号
            title_div = card.find(
                "div", class_="p-6 bg-gray-50 border-b border-gray-200"
            )
            if not title_div:
                continue
            number_span = title_div.find(
                "span",
                class_="inline-flex items-center justify-center w-8 h-8 mr-3 rounded-full bg-gray-100 text-gray-700 text-sm font-semibold leading-none",
            )
            if not number_span:
                continue
            number = number_span.get_text(strip=True)

            # 提取图片元素
            img_div = card.find(
                "div",
                class_="relative w-full h-64 md:h-auto md:flex-1 md:min-h-[400px] bg-gray-100 order-1 md:order-2 border-b md:border-b-0 md:border-l border-gray-200",
            )
            if not img_div:
                continue
            img = img_div.find("img")
            if not img:
                continue

            src = img.get("src")
            if not src:
                srcset = img.get("srcset")
                if srcset:
                    src = srcset.split(",")[0].split()[0]
                else:
                    logger.info(f"跳过序号 {number}: 未找到图片URL")
                    continue

            # 解析 Next.js 图片路径
            parsed = urllib.parse.urlparse(src)
            if parsed.path == "/_next/image":
                query_params = urllib.parse.parse_qs(parsed.query)
                if "url" in query_params:
                    encoded_url = query_params["url"][0]
                    real_path = urllib.parse.unquote(encoded_url)
                else:
                    logger.info(f"序号 {number}: 未找到url参数，跳过")
                    continue
            else:
                real_path = src

            full_img_url = urllib.parse.urljoin(BASE_URL, real_path)
            ext = os.path.splitext(real_path)[1]
            if not ext:
                ext = ".jpg"
            filename = f"{number}{ext}"
            filepath = os.path.join(output_dir, filename)

            if os.path.exists(filepath):
                logger.info(f"图片 {filename} 已存在，跳过")
                continue

            try:
                async with session.get(
                    full_img_url,
                    headers=headers,
                    timeout=aiohttp.ClientTimeout(total=10),
                ) as img_response:
                    img_response.raise_for_status()
                    img_data = await img_response.read()
                with open(filepath, "wb") as f:
                    f.write(img_data)
                logger.info(f"已下载: {filename} ({full_img_url})")
            except Exception as e:
                logger.info(f"下载失败 {filename}: {e}")


def extract_random_questions(json_file_path, num=10):
    """
    从JSON题库文件中随机抽取指定数量的题目，
    返回一个包含问题、自然格式选项、答案字母和图片地址的字典列表。
    """
    if not os.path.exists(json_file_path):
        raise FileNotFoundError(f"题库文件不存在：{json_file_path}")

    try:
        # 使用 utf-8-sig 自动处理 BOM
        with open(json_file_path, "r", encoding="utf-8-sig") as f:
            questions = json.load(f)
    except json.JSONDecodeError as e:
        # 打印详细错误信息，便于调试
        logger.error(f"JSON 解析错误: {e}")
        logger.error(f"错误位置: 第 {e.lineno} 行, 第 {e.colno} 列")
        # 打印上下文
        with open(json_file_path, "r", encoding="utf-8-sig") as f:
            lines = f.readlines()
            start = max(0, e.lineno - 2)
            end = min(len(lines), e.lineno + 2)
            logger.error("上下文：")
            for i in range(start, end):
                prefix = ">>> " if i == e.lineno - 1 else "    "
                logger.error(prefix + lines[i].rstrip())
        raise  # 重新抛出异常，让上层处理

    if not isinstance(questions, list):
        raise ValueError("JSON 文件根元素应为数组")

    # 随机抽取
    if len(questions) < num:
        selected = questions
    else:
        selected = random.sample(questions, num)

    result = []
    for q in selected:
        # 验证必要字段
        if "options" not in q or "correct_letter" not in q:
            logger.warning(f"警告：题目 {q.get('id', 'unknown')} 缺少必要字段，跳过")
            continue
        option_strings = [f"{opt['letter']}. {opt['text']}" for opt in q["options"]]
        result.append(
            {
                "id": q.get("id", 0),
                "question": q.get("question", ""),
                "options": option_strings,
                "correct": q["correct_letter"],
                "image": q.get("image", ""),
            }
        )
    return result


def _load_user_data(data_root: str) -> Dict:
    """
    加载用户数据文件。
    :param data_root: 插件数据根目录（如 data/plugin_data/astrbot_plugin_quiz）
    :return: 用户数据字典，格式为 {"users": {"user_id": {"quiz_type": {...}}}}
    """
    file_path = os.path.join(data_root, "user_data.json")
    if not os.path.exists(file_path):
        return {"users": {}}
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            return json.load(f)
    except (json.JSONDecodeError, IOError):
        # 如果文件损坏，返回空数据
        return {"users": {}}


def _save_user_data(data_root: str, data: Dict) -> None:
    """
    保存用户数据到文件。
    :param data_root: 插件数据根目录
    :param data: 要保存的数据字典
    """
    file_path = os.path.join(data_root, "user_data.json")
    # 确保目录存在
    os.makedirs(os.path.dirname(file_path), exist_ok=True)
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


import logging

# 或者 from astrbot.api import logger  (根据你的环境选择)
logger = logging.getLogger(__name__)


def update_user_money(
    data_root: str,
    user_id: str,
    user_name: str,
    quiz_type: str,
    current_game_money: int,
) -> Tuple[int, int]:
    """
    更新用户的总奖金和最高纪录，并记录最新的用户名。
    :param data_root: 插件数据根目录
    :param user_id: 用户唯一标识（如 QQ 号、平台 ID）
    :param user_name: 用户当前昵称，用于记录最新用户名
    :param quiz_type: 题目类型（如 "math", "history"）
    :param current_game_money: 本次游戏获得的奖金（最终奖池金额）
    :return: (total_money, highest_record) 更新后的总奖金和该类型最高纪录
    """
    try:
        data = _load_user_data(data_root)
        # 确保用户存在
        if user_id not in data["users"]:
            data["users"][user_id] = {}
        user = data["users"][user_id]

        # 更新用户名（如果改变则覆盖）
        user["name"] = user_name

        # 确保该题目类型存在
        if quiz_type not in user:
            user[quiz_type] = {"total_money": 0, "highest_record": 0}

        stats = user[quiz_type]
        # 累加总奖金
        stats["total_money"] += current_game_money
        # 更新最高纪录（如果本次奖金更高）
        if current_game_money > stats["highest_record"]:
            stats["highest_record"] = current_game_money

        # 保存数据
        _save_user_data(data_root, data)

        return stats["total_money"], stats["highest_record"]

    except Exception as e:
        # 记录详细错误信息，便于排查
        logger.error(
            f"更新用户数据失败: {e}\n"
            f"user_id={user_id}, quiz_type={quiz_type}, current_game_money={current_game_money}"
        )
        # 返回安全的默认值，避免调用方解包 None 导致崩溃
        return 0, 0


def get_user_stats(data_root: str, user_id: str, quiz_type: str) -> Tuple[int, int]:
    """
    获取用户在该题目类型下的总奖金和最高纪录。
    :param data_root: 插件数据根目录
    :param user_id: 用户唯一标识
    :param quiz_type: 题目类型
    :return: (total_money, highest_record) 如果用户无记录则返回 (0, 0)
    """
    data = _load_user_data(data_root)
    user = data["users"].get(user_id, {})
    stats = user.get(quiz_type, {"total_money": 0, "highest_record": 0})
    return stats["total_money"], stats["highest_record"]


def get_user_all_stats(data_root: str, user_id: str) -> Dict[str, Dict[str, int]]:
    data = _load_user_data(data_root)
    user_data = data["users"].get(user_id, {})

    stats_dict = {}
    for key, value in user_data.items():
        if (
            isinstance(value, dict)
            and "total_money" in value
            and "highest_record" in value
        ):
            stats_dict[key] = value
    return stats_dict


def get_all_users_stats(data_root: str) -> Dict[str, Dict[str, Dict[str, int]]]:
    """
    获取所有用户的统计信息（过滤掉非统计字段，如 name）。
    :param data_root: 插件数据根目录
    :return: 字典，格式为 {
        "user_id": {
            "quiz_type1": {"total_money": 100, "highest_record": 50},
            "quiz_type2": {"total_money": 200, "highest_record": 150}
        },
        ...
    }
    若某用户无任何统计记录，则不包含在返回结果中。
    """
    data = _load_user_data(data_root)
    all_stats = {}
    for uid, user_data in data["users"].items():
        stats_dict = {}
        for key, value in user_data.items():
            # 只保留符合条件的统计字典
            if (
                isinstance(value, dict)
                and "total_money" in value
                and "highest_record" in value
            ):
                stats_dict[key] = value
        # 只有至少有一个统计类型的用户才加入
        if stats_dict:
            all_stats[uid] = stats_dict
    return all_stats


logger = logging.getLogger(__name__)


def list_available_quizzes(validate_content: bool = False):
    """
    列出所有已下载并保存的可用测验题库。

    该函数扫描插件数据目录（AstrBot 的 data/plugin_data/astrbot_plugin_quiz），
    查找所有以 '_quiz.json' 结尾的文件，将其视为一个可用的题库。

    Args:
        validate_content (bool): 是否对 JSON 文件内容进行基本校验（如是否为列表、至少包含一道题目）。
                                 默认为 False，仅检查文件存在性。设为 True 时会读取并验证文件内容，
                                 但会增加 I/O 开销。

    Returns:
        list[dict]: 每个字典包含以下字段：
            - quiz_type (str): 题库类型，例如 'marvel', 'bleach'。
            - json_file (str): JSON 文件的绝对路径。
            - num_questions (int): 题目数量（仅在 validate_content=True 时存在，否则为 0）。
            - valid (bool): 内容是否有效（仅在 validate_content=True 时存在）。
    """
    # 计算插件数据目录（与 download_images 中的路径计算方式一致）
    plugin_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.dirname(
        os.path.dirname(os.path.dirname(plugin_dir))
    )  # AstrBot 根目录
    data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")

    if not os.path.isdir(data_root):
        logger.warning(f"数据目录不存在: {data_root}")
        return []

    quizzes = []
    for filename in os.listdir(data_root):
        if filename.endswith("_quiz.json"):
            json_path = os.path.join(data_root, filename)
            quiz_type = filename[:-9]  # 去掉 '_quiz.json'

            # 默认认为文件存在即可
            result = {
                "quiz_type": quiz_type,
                "json_file": json_path,
                "num_questions": 0,
                "valid": True,
            }

            if validate_content:
                # 尝试读取并校验内容
                try:
                    with open(json_path, "r", encoding="utf-8-sig") as f:
                        data = json.load(f)
                    if isinstance(data, list) and len(data) > 0:
                        result["num_questions"] = len(data)
                        result["valid"] = True
                    else:
                        result["valid"] = False
                        logger.warning(f"题库 {quiz_type} 内容格式错误或为空")
                except Exception as e:
                    result["valid"] = False
                    logger.warning(f"读取题库 {quiz_type} 失败: {e}")
            else:
                # 不校验内容时，只记录文件存在
                result["valid"] = True

            quizzes.append(result)

    return quizzes


def get_available_quiz_names(validate_content: bool = False):
    """
    仅返回可用题库的名称列表（字符串）。
    """
    quizzes = list_available_quizzes(validate_content=validate_content)
    names = [q["quiz_type"] for q in quizzes if q["valid"]]
    # 剔除每个名称中的 [] '' "" _
    cleaned_names = []
    for name in names:
        # 移除方括号
        name = name.replace("[", "").replace("]", "")
        # 移除单引号
        name = name.replace("'", "")
        # 移除双引号
        name = name.replace('"', "")
        # 移除下划线
        name = name.replace("_", "")
        cleaned_names.append(name)
    return cleaned_names


@register("Quiz", "cyh-x", "一个基于FoxQuiz网站的知识问答插件", "1.0.0")
class MyPlugin(Star):
    def __init__(self, context: Context):
        super().__init__(context)

    async def initialize(self):
        """可选择实现异步的插件初始化方法，当实例化该插件类之后会自动调用该方法。"""

    @filter.command("quiz")
    async def quiz(self, event: AstrMessageEvent, type_of_quiz: str):
        """输入/quiz [题目类型]来开始问答"""
        user_name = event.get_sender_name()
        user_id = (
            event.get_sender_id()
        )  # 假设存在此方法，若没有可改为 event.get_sender() 或其他唯一标识
        quiz_turn = 0
        money = 0
        chapter = 0

        # 当前插件所在目录（如 /home/cyh/AstrBot/data/plugins/astrbot_plugin_quiz）
        plugin_dir = os.path.dirname(os.path.abspath(__file__))
        # 向上三级到达 AstrBot 根目录（因为插件目录在 data/plugins/插件名）
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(plugin_dir)))
        # 目标数据根目录：AstrBot/data/plugin_data/astrbot_plugin_quiz
        data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")
        os.makedirs(data_root, exist_ok=True)

        file_path = os.path.join(data_root, f"{type_of_quiz}_quiz.json")

        # 获取用户历史数据
        total_money, highest_record = get_user_stats(data_root, user_id, type_of_quiz)

        # 发送欢迎消息（可附带历史记录）
        yield event.plain_result(
            f"欢迎{user_name}参加{type_of_quiz}个人问答挑战，共10题，每道题目奖金依次升高。但如果答错题目，奖金将清零！\n"
            f"您的累计奖金：{total_money} 元，本类型最高纪录：{highest_record} 元。"
        )

        try:
            random_questions = extract_random_questions(file_path, 10)
        except FileNotFoundError:
            yield event.plain_result(
                f"题库文件不存在：{file_path}。请确认是否下载了该题库，或者尝试使用unload删除题库后重新下载"
            )
            logger.error(
                f"题库文件不存在：{file_path}。请确认是否下载了该题库，或者尝试使用unload删除题库后重新下载"
            )
            return
        except json.JSONDecodeError:
            yield event.plain_result(
                "题库文件格式错误，请检查 JSON 格式。或者尝试使用unload删除题库后重新下载"
            )
            logger.error(
                f"题库文件格式错误，请检查 JSON 格式。或者尝试使用unload删除题库后重新下载"
            )
            return

        if random_questions:
            img_path = os.path.join(
                data_root,
                f"{type_of_quiz}_quiz_images",
                f"{random_questions[quiz_turn]['id']}.png",
            )
            text_content = f"请在15秒内回答第1题：{random_questions[quiz_turn]['question']}\n{random_questions[quiz_turn]['options']}"

            if os.path.exists(img_path):
                chain = [
                    Comp.Plain(text_content),
                    Comp.Image.fromFileSystem(img_path),
                ]
                yield event.chain_result(chain)
            else:
                logger.warning(
                    f"图片不存在，可能是foxquiz官方网站没有提供图片，跳过图片: {img_path}"
                )
                yield event.plain_result(text_content)
        else:
            yield event.plain_result("题库为空。")
            return

        @session_waiter(timeout=15, record_history_chains=False)
        async def empty_mention_waiter(
            controller: SessionController, event: AstrMessageEvent
        ):
            nonlocal quiz_turn, money, chapter
            current_q = random_questions[quiz_turn]
            answer = event.message_str

            if chapter == 1:
                if answer == "退出" or answer == "quit":
                    # 退出时也更新用户数据（当前已获得奖金）
                    new_total, new_highest = update_user_money(
                        data_root, user_id, user_name, type_of_quiz, money
                    )
                    await event.send(
                        event.plain_result(
                            f"游戏结束，恭喜您本次挑战获得{money}元奖金。\n"
                            f"该类型题目累计总奖金：{new_total} 元，{type_of_quiz} 类型最高纪录：{new_highest} 元。"
                        )
                    )
                    controller.stop()
                    return

                else:
                    # 发送下一题
                    try:
                        next_q = random_questions[quiz_turn]
                        text_content = f"第{quiz_turn + 1}题：{next_q['question']}\n{next_q['options']}"
                        img_path = os.path.join(
                            data_root,
                            f"{type_of_quiz}_quiz_images",
                            f"{random_questions[quiz_turn]['id']}.png",
                        )

                        # 检查图片是否存在
                        if os.path.exists(img_path):
                            chain = [
                                Comp.Plain(text_content),
                                Comp.Image.fromFileSystem(img_path),
                            ]
                            await event.send(event.chain_result(chain))
                        else:
                            logger.warning(f"图片不存在，仅发送文本: {img_path}")
                            await event.send(event.plain_result(text_content))

                        controller.keep(timeout=15, reset_timeout=True)
                        chapter = 0
                        return
                    except Exception as e:
                        # 这里可以捕获其他意外错误
                        logger.error(f"发送题目时出错: {e}")
                        await event.send(f"发送题目失败，请稍后重试。")

            else:
                if answer.strip().upper() == str(current_q["correct"]).upper():
                    money += 50 * (quiz_turn + 1)
                    quiz_turn += 1

                    if quiz_turn == 10:
                        # 全部答对，更新用户数据
                        new_total, new_highest = update_user_money(
                            data_root, user_id, user_name, type_of_quiz, money
                        )
                        await event.send(
                            event.plain_result(
                                f"挑战成功！获得全部奖金，奖池有{money}元！\n"
                                f"该类型累计总奖金：{new_total} 元，{type_of_quiz} 类型最高纪录：{new_highest} 元。"
                            )
                        )
                        controller.stop()
                        return
                    else:
                        await event.send(
                            event.plain_result(
                                f"奖池里面已经有{money}元，还要继续吗？可以输入“退出”或“quit”来带走已经获得的奖金，除此以外任何输入都视作继续挑战！"
                            )
                        )
                        controller.keep(timeout=15, reset_timeout=True)
                        chapter = 1
                        return

                else:
                    # 答错，更新用户数据（当前已获得奖金）
                    money = 0
                    new_total, new_highest = update_user_money(
                        data_root, user_id, user_name, type_of_quiz, money
                    )
                    await event.send(
                        event.plain_result(
                            f"回答错误!本次挑战结束，一共回答正确了{quiz_turn}个问题，奖池{money}元已经清零。\n"
                            f"您的该类型累计总奖金：{new_total} 元，{type_of_quiz} 类型最高纪录：{new_highest} 元。再接再厉！"
                        )
                    )
                    controller.stop()
                    return

        try:
            await empty_mention_waiter(event)
        except TimeoutError as _:
            # 超时，更新用户数据（当前已获得奖金）
            money = 0
            new_total, new_highest = update_user_money(
                data_root, user_id, user_name, type_of_quiz, money
            )
            yield event.plain_result(
                f"你超时了！本次挑战结束，本次奖金已经清零。\n"
                f"累计总奖金：{new_total} 元，{type_of_quiz} 类型最高纪录：{new_highest} 元。"
            )
        except Exception as e:
            yield event.plain_result("发生错误，请联系管理员: " + str(e))
        finally:
            event.stop_event()

    @filter.command("quiz_load")
    async def quiz_load(self, event: AstrMessageEvent, type_of_quiz: str):
        """下载知识问答题库到本地"""
        # 1. 校验 type_of_quiz 合法性
        # 禁止为空、包含路径分隔符或特殊字符，避免路径遍历攻击
        if not type_of_quiz or not isinstance(type_of_quiz, str):
            yield event.plain_result("参数错误：题型名称不能为空。")
            return
        # 禁止路径遍历字符
        if any(c in type_of_quiz for c in ["/", "\\", "..", ".", "\0"]):
            yield event.plain_result("题型名称包含非法字符，请使用字母、数字或下划线。")
            return

        # 当前插件所在目录
        plugin_dir = os.path.dirname(os.path.abspath(__file__))

        # 向上三级到达 AstrBot 根目录（因为插件目录在 data/plugins/插件名）
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(plugin_dir)))

        # 目标数据根目录：AstrBot/data/plugin_data/astrbot_plugin_quiz
        data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")

        try:
            # 确保目录存在（父目录也会自动创建）
            os.makedirs(data_root, exist_ok=True)
        except Exception as e:
            yield event.plain_result(f"创建数据目录失败: {str(e)}")
            return

        # 构建各个文件的完整路径
        html_file = os.path.join(data_root, f"{type_of_quiz}_quiz.html")
        json_file = os.path.join(data_root, f"{type_of_quiz}_quiz.json")
        images_dir = os.path.join(data_root, f"{type_of_quiz}_quiz_images")

        # 2. 下载题目信息 (JSON)
        try:
            await download_and_parse_quiz(
                quiz_type=type_of_quiz, html_file=html_file, json_file=json_file
            )
        except Exception as e:
            yield event.plain_result(f"下载题库失败: {str(e)}")
            return

        # 3. 下载图片
        try:
            await download_images(quiz_type=type_of_quiz, output_dir=images_dir)
        except Exception as e:
            yield event.plain_result(f"下载图片失败: {str(e)}")
            return

        yield event.plain_result(f"{type_of_quiz}题库下载完成")

    @filter.command("quiz_unload")
    async def quiz_unload(self, event: AstrMessageEvent, quiz_type: str):
        """
        删除指定 quiz_type 的题库数据：HTML 文件、JSON 文件和图片文件夹。

        Args:
        quiz_type (str): 测验类型，例如 'marvel', 'bleach' 等。
        """
        # 计算数据根目录（与 quiz_load 中逻辑一致）
        plugin_dir = os.path.dirname(os.path.abspath(__file__))
        base_dir = os.path.dirname(
            os.path.dirname(os.path.dirname(plugin_dir))
        )  # AstrBot 根目录
        data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")

        # 构建待删除的文件和目录路径
        html_file = os.path.join(data_root, f"{quiz_type}_quiz.html")
        json_file = os.path.join(data_root, f"{quiz_type}_quiz.json")
        images_dir = os.path.join(data_root, f"{quiz_type}_quiz_images")

        # 删除 HTML 文件
        if os.path.isfile(html_file):
            os.remove(html_file)
            logger.info(f"已删除 HTML 文件: {html_file}")
        else:
            logger.info(f"HTML 文件不存在，跳过: {html_file}")

        # 删除 JSON 文件
        if os.path.isfile(json_file):
            os.remove(json_file)
            logger.info(f"已删除 JSON 文件: {json_file}")
        else:
            logger.info(f"JSON 文件不存在，跳过: {json_file}")

        # 删除图片文件夹（如果存在）
        if os.path.isdir(images_dir):
            shutil.rmtree(images_dir)
            logger.info(f"已删除图片文件夹: {images_dir}")
        else:
            logger.info(f"图片文件夹不存在，跳过: {images_dir}")
        yield event.plain_result(f"{quiz_type}题库卸载完成")

    # 在 MyPlugin 类的适当位置添加（例如在 quiz 方法之后）

    @filter.command_group("quiz_stats")
    def quiz_stats(self):
        pass

    @quiz_stats.command("get")
    async def user_stats(self, event: AstrMessageEvent, target: str = None):
        """
        查询用户统计信息。
        使用方式：/stats [@用户]  （如果不指定则查询自己）
        """
        # 获取目标用户ID（这里简单处理，如果提供了target参数则使用，否则为自己）
        if target:
            # 如果 target 是 @ 提及，可以解析出用户ID，此处假设直接传入数字ID或昵称
            # 为了简化，我们先仅支持查询自己
            user_id = target
            user_name = target  # 暂用传入的值作为显示名
        else:
            user_id = event.get_sender_id()
            user_name = event.get_sender_name()

        # 获取数据目录
        plugin_dir = os.path.dirname(os.path.abspath(__file__))
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(plugin_dir)))
        data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")
        os.makedirs(data_root, exist_ok=True)

        # 获取用户所有统计数据
        all_stats = get_user_all_stats(data_root, user_id)

        if not all_stats:
            yield event.plain_result(f"用户 {user_name} 暂无任何游戏记录。")
            return

        # 构建输出文本
        lines = [f"📊 用户：{user_name} 的问答统计", "=" * 30]
        for quiz_type, stats in all_stats.items():
            lines.append(f"【{quiz_type}】")
            lines.append(f"  累计奖金：{stats['total_money']} 元")
            lines.append(f"  最高纪录：{stats['highest_record']} 元")
            lines.append("-" * 20)
        # 移除最后一个分隔线（可选）
        if lines[-1] == "-" * 20:
            lines.pop()
        result = "\n".join(lines)
        total_all = sum(stats["total_money"] for stats in all_stats.values())

        yield event.plain_result(result)
        yield event.plain_result(f"总奖金{total_all}元")

    @quiz_stats.command("rank")
    async def user_rank(self, event: AstrMessageEvent, quiz_type: str = "all"):
        """
        查询用户排行榜。
        使用方式：/rank [类型]  类型可选：all（总奖金榜）、nba、naruto 等，默认 all
        """
        # 获取数据目录
        plugin_dir = os.path.dirname(os.path.abspath(__file__))
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(plugin_dir)))
        data_root = os.path.join(base_dir, "data", "plugin_data", "astrbot_plugin_quiz")
        os.makedirs(data_root, exist_ok=True)

        # 获取所有用户的统计信息（已过滤）
        users = get_all_users_stats(data_root)
        if not users:
            yield event.plain_result("暂无用户数据。")
            return

        # 准备排行榜列表
        rank_list = []
        for uid, stats in users.items():
            if quiz_type == "all":
                # 计算所有类型的总奖金
                total = sum(stat.get("total_money", 0) for stat in stats.values())
            else:
                # 获取指定类型的奖金
                type_stats = stats.get(quiz_type)
                total = type_stats.get("total_money", 0) if type_stats else 0
            if total > 0:
                rank_list.append((uid, total))

        # 按总奖金降序排序
        rank_list.sort(key=lambda x: x[1], reverse=True)

        if not rank_list:
            yield event.plain_result(f"暂无【{quiz_type}】类型的有奖记录。")
            return

        # 构建排行榜文本（取前10名）
        top_n = 10
        lines = [f"🏆 {quiz_type.upper()} 排行榜"]
        lines.append("=" * 20)
        for idx, (uid, total) in enumerate(rank_list[:top_n], 1):
            # 尝试获取用户昵称（可使用 event 或全局缓存，这里简化为显示 ID）
            # 可以尝试从 event 获取发送者的昵称，但这里无法批量获取，故只显示 ID
            lines.append(f"{idx}. 用户 ({uid})：{total} 元")
        if len(rank_list) > top_n:
            lines.append(f"... 共 {len(rank_list)} 人，仅显示前 {top_n} 名")
        result = "\n".join(lines)
        yield event.plain_result(result)

    @filter.command("quiz_list")
    async def quiz_list(self, event: AstrMessageEvent):
        """
        列出所有已下载并保存的可用测验题库。
        """
        yield event.plain_result(f"可用题库如下{get_available_quiz_names()}")

    async def terminate(self):
        """可选择实现异步的插件销毁方法，当插件被卸载/停用时会调用。"""
